...
create_cloned_data_set <- function(
	...
	if("death_for_tb" %in% endpoint){
		weekly_records_death_for_tb_z1 <- propensity_output$weekly_records_data[
		  ((tpt_start_wk <= propensity_output$grace_pd_wks) & (wk <= min(death_wk, right_cens_wk_tb)) & (wk <= min(tb_wk, right_cens_wk_tb))) |
	    ((tpt_start_wk > propensity_output$grace_pd_wks) & (wk <= propensity_output$grace_pd_wks) & (wk <= min(death_wk, right_cens_wk_tb)) & (wk <= min(tb_wk, right_cens_wk_tb)))
	  ]
	...
	if("tb" %in% endpoint){
		weekly_records_tb_z0 <- propensity_output$weekly_records_data[
		  ((tpt_start_wk < 99999) & (wk <= tpt_start_wk) & (wk <= min(tb_wk, right_cens_wk_tb))) |
		  ((tpt_start_wk == 99999) & (wk <= min(tb_wk, right_cens_wk_tb)))
		]
	...
	if("death_for_tb" %in% endpoint){
		weekly_records_death_for_tb_z0 <- propensity_output$weekly_records_data[
		  ((tpt_start_wk < 99999) & (wk <= tpt_start_wk)) & (wk <= min(death_wk, right_cens_wk_tb)) & (wk <= min(tb_wk, right_cens_wk_tb)) |
